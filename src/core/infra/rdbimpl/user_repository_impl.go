...
func (repo *userRepositoryImpl) Store(ctx context.Context, userdomain *userdm.UserDomain) error {
	...
	if err != nil {
		return database_errors.ErrDatabaseError(fmt.Sprintf("Failed to store user: %v", err))
	}
	...
	for _, skill := range userdomain.Skills {
		...
		if err != nil {
			return database_errors.ErrDatabaseError(fmt.Sprintf("Failed to store skill: %v", err))
		}
	}
	...
	for _, career := range userdomain.Careers {
		...
		if err != nil {
			return database_errors.ErrDatabaseError(fmt.Sprintf("Failed to store career: %v", err))
		}
	}
...
func (repo *userRepositoryImpl) FindByName(ctx context.Context, name string) (*userdm.User, error) {
	...
	if err != nil {
		...
		return nil, database_errors.ErrDatabaseError(fmt.Sprintf("user_repository FindByName database error: %v", err))
	}
	...
	if err != nil {
		return nil, domainErrors.ErrReconstructionError(fmt.Sprintf("error reconstructing user from userRequest: %v", err))
	}
...
func (repo *userRepositoryImpl) FindByNames(ctx context.Context, names []string) (map[string]*userdm.User, error) {
	...
	if err != nil {
		return nil, database_errors.ErrDatabaseError(fmt.Sprintf("query construction error: %v", err))
	}

	err = repo.Conn.Select(&tempUsers, query, args...)
	if err != nil {
		return nil, database_errors.ErrDatabaseError(fmt.Sprintf("query execution error: %v", err))
	}
	...
	for _, tempUser := range tempUsers {
		user, err := userdm.Reconstruct(tempUser.ID, tempUser.Name, tempUser.Email, tempUser.Password, tempUser.Profile, tempUser.CreatedAt)
		if err != nil {
			return nil, domainErrors.ErrReconstructionError(fmt.Sprintf("error converting userRequest to User: %v", err))
		}
